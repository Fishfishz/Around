...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	//* means including everything, return the header for every front application.
	w.Header().Set("Access-Control-Allow-Origin", "*")
	w.Header().Set("Access-Control-Allow-Headers", "Content-Type,Authorization")
	//broswer will send OPTIONS request first to see whether the header is correct
	//during the cross domain request
	if r.Method == "OPTIONS" {
		return
	}
	...
	//annotate imgae with vision api
	if p.Type == "image" {
	...
	fmt.Printf("Post is saved to Elasticsearch")
}
...
func saveToES(i interface{}, index string, id string) error {
	...
	_, err = client.Index(). //this Index means Insert
					Index(index). //this Index means database name
					Id(id).
					BodyJson(i).
					Do(context.Background())
